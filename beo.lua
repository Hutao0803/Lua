...
local function GetMob()
    ...
    local Lv = CheckQuest().LevelReq
    for i, v in pairs(LvTab) do
        if i == Lv then
            Lv = v
        end
    end 
    return CheckQuest().MobName .. " [Lv. " .. Lv .. "]"                   
end
local bindable = Instance.new("BindableFunction")
local function gui()
    game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Hirimi Hub";Text = "Farm";Icon = "rbxassetid://14299284116";Duration = math.huge;Callback = bindable;Button1 = "Bật";Button2 = "Tắt"})
end
gui()
...
local function FastFarm()
    ...
    if Enemies:FindFirstChild("Shanda [Lv. 475]") then     
        for i,v in pairs(Enemies:GetChildren()) do
            if v.Name == "Shanda [Lv. 475]" and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0  and v.Parent then
            ...
            TP(MobPos("Shanda")[Pos] * CFrame.new(getgenv().PosX, getgenv().PosY, getgenv().PosZ))
            Pos = Pos + 1
            wait(1)
        end 
end
...
local function GoToSea2()
    ...
    if Remote:InvokeServer("DressrosaQuestProgress").UsedKey == false then
        ...
        if Remote:InvokeServer("DressrosaQuestProgress").KilledIceBoss == false then
            if b:FindFirstChild("Ice Admiral [Lv. 700] [Boss]") then
                for i, v in pairs(b:GetChildren()) do
                     if v.Name == "Ice Admiral [Lv. 700] [Boss]" and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 and v.Parent then
                         EquipTool(getgenv().SelectedWeapon)                                                                                                                    
                         Buso()
                         NameMob = v.Name
                         PosMon = v.HumanoidRootPart.CFrame
...
if getgenv().Kaitun and LP.Character and LP.Character:FindFirstChild("Humanoid") then
        ...
        elseif mylevel >= 700 and game.PlaceId == 2753915549 then
            ...
            if Quest.Visible == true then
                if not string.find(QuestTitle, CheckQuest()["MobName"]) then
                    ...
                    if Enemies:FindFirstChild(GetMob()) then     
                        for i,v in pairs(Enemies:GetChildren()) do
                            if getgenv().Kaitun and v.Name == GetMob() and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0  and v.Parent and Quest.Visible == true then
                                EquipTool(getgenv().SelectedWeapon)                                                                                                                    
                                Buso()
                                  NameMob = v.Name
                                PosMon = v.HumanoidRootPart.CFrame                                                                                                   
